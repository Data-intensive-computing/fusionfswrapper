 ...
 *
 * Update history:
 *	1/20/2015, so refactoring the code?
 * 	07/19/2012:
...
JNI specifies a communication protocol between Java code and external, native code.
It enables  Java code i.e wrapperclass to interface with native code written in c
Java native methods are methods declared in your Java cwrapper (much like you declare an abstract method), but which are actually implemented in here so below are the three methods 

*/
JNIEXPORT jint JNICALL Java_FusionWrap_fusion_1read(JNIEnv *env, jclass cls, jstring path) 
{
char *buf;
size_t size;
off_t offset;
struct fuse_file_info *fi
return fusion_read(path, buf, size, offset,fi);
}
JNIEXPORT jint JNICALL Java_FusionWrap_fusion_1write(JNIEnv *env, jclass cls, jstring path) 
{
char *buf;
size_t size;
off_t offset;
struct fuse_file_info *fi
return fusion_write(path,buf,size,offset,fi);
}
JNIEXPORT jint JNICALL Java_FusionWrap_fusion_1getattr(JNIEnv *env, jclass cls, jstring path) 
{
struct stat *statbuf
return fusion_getattr(path,statbuf);
}
...
int zht_update(const char *key, const char *val)
{
	...
	/*
	 * the following code shoule be commented out... it's huge overhead
	 */
...
/**
 * DFZ: this is a strange operation... at least not as simple as it seems.
 * 		It is actually an update with a partial removal
 * <key, oldval> -> <key, (oldval - val)>
 */
...
int fusion_getattr(const char *path, struct stat *statbuf)
{
	...
	{ 
		...
		/*if path is an existing directory
			we need to make sure the directory is reflected in the local ext4
			
			NOTE: we are not talking about regular files, but directories.
		*/
 ...
 * 		2) don't touch the remote file when updating the meta data, but do a batch job in each node to self-check dangling files
 */
int fusion_unlink(const char *path)
{
	...
	/*
		DFZ: not sure about the purpose of first two lines; are they 
		for debug? That is, don't try to remove something that doesn't exist.
			---It's for later handling of remote stale copy
	*/
...
int fusion_release(const char *path, struct fuse_file_info *fi)
{
	...
	/* MACROs for file mode:
		O_ACCMODE<0003>\A3\BAaccess the lower 2 bits in file I/O flag
		O_RDONLY<00>\A3\BAread only
		O_WRONLY<01>\A3\BAwrite only
		O_RDWR<02>\A3\BAread and write
	 */
	...
	/*otherwise, dealing with the remote copy*/
	...
	{	/* we don't want to keep a redundant copy in local node to
...
int fusion_opendir(const char *path, struct fuse_file_info *fi)
{
	...
	if (ZHT_LOOKUP_FAIL != stat) /*map the directory to this node*/
	{
		mkdir(fpath, 0775);
	}
	else 
		...
		off_t offset, struct fuse_file_info *fi)
{
	...
	while (pch) 
	{
		 ...
		 * DFZ: are they really necessary (to map to the local ext4)?
		 */
		if ('/' == *(pch + strlen(pch) - 1)) {
...
int fusion_create(const char *path, mode_t mode, struct fuse_file_info *fi)
{
	...
	zht_append(dirname, pch + 1); /*so a "ls" only return the relative file names?*/
...
void fusion_usage() {
	fprintf(stderr, "usage:  fusionfs rootDir mountPoint neighbor_conf zht_conf\n");
	abort();
}
...
int main(int argc, char *argv[]) 
{
	...
	//DFZ: this can be commented out if needed
	...
	/*DFZ: added for ZHT conf files*/
	char *zhtconf = realpath(argv[argc-1], NULL);
	char *neighborconf = realpath(argv[argc-2], NULL);
	argc -= 2;
